...
extractPreciAccurBeds<-function(folder,obsName,theCodes1,variant,type){
  ...
  if (!theCodes1 %in% c("localTUEcodes","BWcodes","allCodes", "localROcodes")){
    fwdAccur$col<-"mismatched BLs"
  }
  else if(theCodes1 %in% c("localTUEcodes", "BWcodes","allCodes", "localROcodes")){
...
plotBeds <- function(folder, allPrAc, variant, obsName, type, accurType, precType, plotCombi=T){
  ...
  AccFwdBeds <- ggplot(bind_rows(lapply(1:length(allPrAc), function(x) allPrAc[[x]]$fwdAccur)))+
    ...
    scale_color_manual(
      limits=c( "mismatched BLs","localHDcodes","BWcodes","allCodes","localROcodes"),
      values=c("grey", "red", "blue","yellow","black"),
    )+
    ...
    theme_light()+
    theme(legend.position = "bottom",legend.text = element_text(size = 10),legend.title = element_blank(), plot.title = element_text(size = 10))
  ...
  PrcFwdBeds <- ggplot(bind_rows(lapply(1:length(allPrAc), function(x) allPrAc[[x]]$fwdPrec)))+
    ...
    scale_color_manual(
      limits=c("mismatched BLs", "localHDcodes", "BWcodes","allCodes","localROcodes"),
      values=c("gray", "red", "blue","yellow","black")
    )+
    scale_x_continuous(expand = c(0,0.5), limits = c(1,30)) +
    scale_y_continuous(limits=c(0,1))+
    xlim(1,30) +
  ...
  AccPrecFwdBeds <- ggplot(allFwdAccPrec)+
    ...
    geom_point(data=subset(allFwdAccPrec,(dayFwd %in% c(7,14,30))&(Codes.x %in% c("localTUEcodes","BWcodes","allCodes","localROcodes"))),aes_string(x=accurType,y="prec",shape="dayFwdFactor"),size=3,color="black",fill="#00000000")+
    ...
    (if(plotCombi == T){
      scale_fill_manual(
        limits=c("mismatched BLs", "localTUEcodes", "BWcodes","allCodes","localROcodes"),
        labels=c("mismatched BLs", localTUEcodes="localcodes","clusterFRcodes","BWcodes","allCodes","localROcodes"),
        values=c("#00000010", "red", "yellow","black","turquoise"))
    }
    else{scale_fill_manual(
        limits=c("mismatched BLs", "localTUEcodes", "BWcodes","allCodes","localROcodes"),
        labels=c("mismatched BLs", localTUEcodes="localHDcodes","clusterFRcodes","BWcodes","allCodes","localROcodes"),
        values=c("#00000010", "red", "yellow","black","turquoise")
      )})+
    (if(plotCombi == T){
    scale_colour_manual(
    ...
    else if(type == "VOC_ETS" & plotCombi == T){labs(title=obsName)}) +
    theme_light()+
  ...
  ggsave(paste0(folder,"Beds/AccPrecFwdBeds-",variant,"-",type,".png"),width=7,height=8)
...
plotBedForecastOverTime <- function(folder, variant, obsName, type){
  ...
  exampleResults<-list(
    extractSingleForecastDate(folder,obsName,"localTUEcodes",variant,type,as.Date("2021-09-14")),
    #extractSingleForecastDate(folder,obsName,"clusterFRcodes",variant,type,as.Date("2021-09-14")),
    extractSingleForecastDate(folder,obsName,"localROcodes",variant,type,as.Date("2021-09-14")),
  ...
  bedPlotIQR <- ggplot(bind_rows(lapply(1:length(exampleResults), function(x) exampleResults[[x]]$forecastResults)))+
    ...
    scale_fill_manual(limits=c("allCodes","BWcodes","localHDcodes"),
                      values=c("#FFFFFF40","#FFFFFF40","#FF000040"))+
    theme_light()+
...
plotRoverTime <- function(folder, variant, obsName, theCodes1, startVOCdateAlpha, startVOCdateDelta, startVOCdateOmicron){
  ...
  RHisto <- readRDS(paste0(folder,"R/RHisto-",obsName,"-",theCodes1,"-",variant,".rds"))
...
calc_mase <- function(folder, variant, theCodes1, obsName){
  ...
  #RD_basedOnA_VOC_loop_allNums <- RunBasedOnCodesLoop(startDataDate, startDate, sameStartDate = F, numSims = numSims, "localFRcodes", accurPred = F, obsName = obsName, inclVOC = T, startVOCdate = startVOCdate, immEvasionVOC = immEvasionVOC, advanVOC = advanVOC, propVOC = propVOC)
  #RD_basedOnA_noVOC_loop_allNums <- RunBasedOnCodesLoop(startDataDate, startDate, sameStartDate = F, numSims = numSims, "localFRcodes", accurPred = F, obsName = obsName, inclVOC = F, startVOCdate = startVOCdate, immEvasionVOC = immEvasionVOC, advanVOC = advanVOC, propVOC = propVOC)
...
### define variables
laenderList<-c("localTUEcodes", "allCodes","MVcodes","SHcodes","HHcodes","NScodes","HBcodes","NRWcodes","HEcodes","RPcodes","BWcodes","BAcodes","SLcodes","BEcodes","BRcodes","SACcodes","SANcodes","THcodes", "localROcodes")
...
if(plotR == T){
  ...
  #accurPrecR(variant="Alpha", theCodes1=allCodesList[1], obsName=obsName)
  #accurPrecR(variant="Alpha", theCodes1=allCodesList[2], obsName=obsName)
...
if(plotInci == T){
  ...
  simpleInciPlot(folder, c("localTUEcodes"), obsName=obsName, variant)
...
if(plotBedsAllCodes == T){
  ...
  assign("AccPrcTUE", gg_plotBedsICU[[4]][[3]])
  save(AccPrcTUE, file="TUEbedsEtsVoCICU.Rdata")
