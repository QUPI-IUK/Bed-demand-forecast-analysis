...
extractPreciAccurBeds<-function(folder,obsName,theCodes1,variant,type){
  ...
  if (!theCodes1 %in% c("localHDcodes","BWcodes","allCodes", "localROcodes")){
    fwdAccur$col<-"mismatched BLs"
  }
  else if(theCodes1 %in% c("localHDcodes", "BWcodes","allCodes", "localROcodes")){
...
plotBeds <- function(folder, allPrAc, variant, obsName, type, accurType, precType, plotCombi=T){
  ...
  AccPrecFwdBeds <- ggplot(allFwdAccPrec)+
    ...
    geom_point(data=subset(allFwdAccPrec,(dayFwd %in% c(7,14,30))&(Codes.x %in% c("localHDcodes","BWcodes","allCodes","localROcodes"))),aes_string(x=accurType,y="prec",shape="dayFwdFactor"),size=3,color="black",fill="#00000000")+
    ...
    (if(plotCombi == T){
      scale_fill_manual(
        limits=c("mismatched BLs", "localHDcodes", "BWcodes","allCodes","localROcodes"),
        labels=c("mismatched BLs", localHDcodes="localcodes","clusterFRcodes","BWcodes","allCodes","localROcodes"),
        values=c("#00000010", "red", "yellow","black","turquoise"))
    }
    else{scale_fill_manual(
        limits=c("mismatched BLs", "localHDcodes", "BWcodes","allCodes","localROcodes"),
        labels=c("mismatched BLs", localHDcodes="localHDcodes","clusterFRcodes","BWcodes","allCodes","localROcodes"),
        values=c("#00000010", "red", "yellow","black","turquoise")
      )})+
    (if(plotCombi == T){
    scale_colour_manual(
    ...
    else if(type == "VOC_ETS" & plotCombi == T){labs(title="HD")}) +
    theme_light()+
...
plotBedForecastOverTime <- function(folder, variant, obsName, type){
  ...
  exampleResults<-list(
    extractSingleForecastDate(folder,obsName,"localHDcodes",variant,type,as.Date("2021-09-14")),
...
### define variables
laenderList<-c("localHDcodes", "allCodes","MVcodes","SHcodes","HHcodes","NScodes","HBcodes","NRWcodes","HEcodes","RPcodes","BWcodes","BAcodes","SLcodes","BEcodes","BRcodes","SACcodes","SANcodes","THcodes", "localROcodes")
...
if(plotInci == T){
  ...
  simpleInciPlot(folder, c("localHDcodes"), obsName=obsName, variant)
...
if(plotBedsAllCodes == T){
  ...
  ### ICU
  ggarrange(gg_ICU[[1]][[1]], gg_plotBedsICU[[2]][[1]], gg_plotBedsICU[[3]][[1]], gg_plotBedsICU[[4]][[1]], ncol = 2, nrow = 2, labels = c("A", "B", "C", "D"), common.legend = TRUE, legend="bottom")
  ...
  assign("AccPrcHD", gg_plotBedsICU[[4]][[3]])
  save(AccPrcHD, file="HDbedsEtsVoCICU.Rdata")
